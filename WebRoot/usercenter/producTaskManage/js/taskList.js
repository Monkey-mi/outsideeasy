...
//         form.submit(); 
	...
	* 2016-9-26
	...
	function gologistics(id){
		...
		window.open(getwebroot()+"externalTask/producLogisticsInfo/"+id+".htm",resultName);
	}
	/**
	 * 任务单页面的接入
	 * @param id
	...
	function gotaskinfo(id){
		var resultName = JSON.stringify({"task_id":id,"companyIdForAll":companyId});		
		window.open(getwebroot()+"externalTask/producTaskInfo/"+id+".htm",resultName);
	}
	/**
	 * 生产单页面的接入
	 * @param id
	...
	function goproducts(id){
		var resultName = JSON.stringify({"task_id":id,"companyIdForAll":companyId});
		window.open(getwebroot()+"externalTask/producProductionInfo/"+id+".htm",resultName);
	}
	/**
	 * 质检单页面的接入
	 * @param id
	...
	function godelevils(id){
		var resultName = JSON.stringify({"task_id":id,"companyIdForAll":companyId});
		window.open(getwebroot()+"externalTask/producDeliveredInfo/"+id+".htm",resultName);
	}
	...
	function goQualityControl(id){
		var resultName = JSON.stringify({"task_id":id,"companyIdForAll":companyId});
		window.open(getwebroot()+"externalTask/producQualityControl/"+id+".htm",resultName);
	}
	/**鼠标经过显示大图
	 * showBigImage
	 * @param e void
