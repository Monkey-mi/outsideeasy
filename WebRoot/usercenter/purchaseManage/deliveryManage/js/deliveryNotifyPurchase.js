...
function getOrderDeliverNotice(pageIndex,needinit){
	...
	var fn = function(result){
		...
		$("#allDliNum").html(counts[0]);
		$("#waitDliNum").html(counts[1]);
		$("#modifyDliNum").html(counts[2]);
		$("#confirmDliNum").html(counts[3]);
		mouseMove();//移入移除样式转换
	};
...
function confirmDeliveryTime(thisEvent,noticeId,updateDt){
	...
	var fnout = function(result){//无操作
	 ...
	 window.wxc.xcConfirm("您要确认该送货通知吗？", window.wxc.xcConfirm.typeEnum.confirm,{	
		onOk:function(){
...
function mouseMove(){
	$(".deliveryNotify_con").mouseover(function(){
		...
		$(this).find(".notifyInfo .left").css("border-right","1px dashed #feebc4");
	});
	$(".deliveryNotify_con").mouseleave(function(){
		...
		$(this).find(".notifyInfo .left").css("border-right","1px dashed #e8e8e8");
	});
}
